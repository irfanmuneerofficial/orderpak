...
@section('script_text')
<?php echo $ccategoriesid->script_text ?? '' ?>
@endsection

<?php
if($ccategoriesid){
  $title = $ccategoriesid->meta_title;
}
else{
  $title = 'Online Shopping in Pakistan with Free Home Delivery';
}
if($ccategoriesid){
  $desc = $ccategoriesid->meta_description;
}
else{
...
if($ccategoriesid->category_img){
  $img = asset('uploads/category/'. $ccategoriesid->category_img);
}
else{
...
@section('page-title')
{{ucwords(str_replace(array('-'),array(' '),Request::segment(4)))}}
@endsection
@section('title')
<?php echo $title ?>
@endsection
...
<script type="application/ld+json">
{
  ...
  "itemListElement": [{
    ...
    "position": 3,
    "item": {
      "@id": "{{ url('category', str_replace(array('/'), array(' '), Request::segment(2))) }}",
    ...
    "position": 4,
    "item": {
      "@id": "{{ url('category/' . Request::segment(2), str_replace(array('/'), array(' '), Request::segment(3))) }}",
    ...
    "position": 5,
    "item": {
      "name": "{{ucwords(str_replace(array('-'), array(' '), Request::segment(4)))}}"
    }
  }]
  ...
  <div class="container max-con">
    <nav aria-label="breadcrumb">
      <ol itemscope itemtype="https://schema.org/BreadcrumbList" class="breadcrumb custom-breadcrumb">
        ...
        <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="breadcrumb-item breadcrumb-item1">
          <a itemprop="item" href="{{ url('category/' . Request::segment(2), str_replace(array('/'), array(' '), Request::segment(3))) }}">
        ...
        <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="breadcrumb-item breadcrumb-item1 active" aria-current="page">
          <span itemprop="name"> {{ucwords(str_replace(array('-'), array(' '), Request::segment(4)))}}</span>
      ...
      <!-- <ol class="breadcrumb custom-breadcrumb">
        ...
        <li class="breadcrumb-item breadcrumb-item1">
          <a href="{{ url('category', str_replace(array('/'), array(' '), Request::segment(2))) }}">
            {{ucwords(str_replace(array('-'), array(' '), Request::segment(2)))}}
          </a>
        </li>
        <li class="breadcrumb-item breadcrumb-item1">
          <a href="{{ url('category/' . Request::segment(2), str_replace(array('/'), array(' '), Request::segment(3))) }}">
    ...
    <div class="row">
      ...
      <div class="col-xl-9 col-lg-9 col-md-9 col-sm-12">
        ...
        <div class="order-border">
                ...
                <div class="thumbnail card product-grid7">
                  <div class="button-product">
                    <div class="img-event product-image7">
                      <img class="pic-1 group list-group-image img-fluid" src={{ asset('uploads/product_images/'.$product->image_1) }} alt="{{$product->title}}" />
                    </div>
                    <div class="caption card-body body-card">
                      <p class="listed">listed by: {{\Illuminate\Support\Str::limit($vendor->business_name, 40, '...')}}</p>
                      <strong class="cat_name_title">({{$product->categoryname->title}})</strong>
                      <hr>
                      <strong class="pcat_title">{{$product->parentcategory->title}}</strong>
                      <strong class="p_title group card-title inner list-group-item-heading">
          ...
          @if (!empty(trim(strip_tags($ccategoriesid->showtext))))
          {!! $ccategoriesid->showtext !!}
          <span id="moredata">
            {!! $ccategoriesid->hidtext !!}
          </span>

            @if (!empty(trim(strip_tags($ccategoriesid->hidtext))))
                <div class="button-product">
